...
#define PENDING_QUEUE_MAXLENGTH 3
...
int checkCredentials(char *login, char *password, char *profil)
{
    ...
    int attempts = 0;
    int isValid = 0;
    while (attempts < MAX_ATTEMPTS && fscanf(f, "%s %s %s", c.login, c.password, c.profil) != EOF)
    {
...
int rechercherContactParGSM(int gsm, char *retour)
{
    ...
    while (fscanf(f, "%s\t%s\t%d\t%s\t%s\t%s\t%s\n", c.nom, c.prenom, &c.GSM, c.email, c.adresse.nomRue, c.adresse.ville, c.adresse.pays) != EOF)
    {
        if (c.GSM == gsm)
        {
            printf("\nContact trouve : %s %s\n", c.nom, c.prenom);
...
char *supprimerContactParGSM(int gsm)
{
    ...
    else
    {
        ...
        sprintf(retour, "Le contact avec le numero GSM specifie a ete supprime avec succes.\n");
